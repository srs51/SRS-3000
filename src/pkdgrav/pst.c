...
#ifdef AGGS
	mdlAddService(mdl,PST_DEMAGGSSETMASS,pst,
				  (void (*)(void *,void *,int,void *,int *)) pstDEMAggsSetMass,
				  sizeof(struct inDEMAggsSetMass),0);
#endif /* AGGS */
...
void pstAggsSetSpaceVel(PST pst,void* vIn,int nIn,void* vOut,int* pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		pkd->PP = &in->PP;
#endif
		pkdAggsSetSpaceVel(pkd,in->iAggIdx,in->v_com,in->omega,in->lambda);
		}
	if (pnOut) *pnOut = 0;
	}
...
void pstDEMWallsReact(PST pst,void *vIn,int nIn,void *vOut,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		local = *out;
		mdlGetReply(pst->mdl,pst->idUpper,vOut,NULL);
		out->dTotalZForceFromParticles += local.dTotalZForceFromParticles;
		}
	else
		pkdDEMWallsReact(pst->plcl->pkd,&out->dTotalZForceFromParticles);
	if (pnOut) *pnOut = sizeof(*out);
	}
...
{
	struct inDEMAggsSetMass *in = vIn;

	mdlassert(pst->mdl,nIn == sizeof(*in));
	if (pst->nLeaves > 1) {
		mdlReqService(pst->mdl,pst->idUpper,PST_DEMAGGSSETMASS,vIn,nIn);
		pstDEMAggsSetMass(pst->pstLower,vIn,nIn,NULL,NULL);
		mdlGetReply(pst->mdl,pst->idUpper,NULL,NULL);
		}
	else
		pkdDEMAggsSetMass(pst->plcl->pkd,in->iAggIdx,in->fMass);
	if (pnOut) *pnOut = 0;
	}
